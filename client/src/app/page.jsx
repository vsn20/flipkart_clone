  ...
  return (
    <div style={{ background: '#f1f3f6', minHeight: '100vh' }}>
      ...
      <header style={{
        ...
        background: isMobile ? 'linear-gradient(180deg, #1a237e 0%, #2d3fa1 8%, #3d4fa8 14%, #4a5fb0 20%, #5a6fb8 28%, rgba(80,100,160,0.5) 36%, rgba(120,140,200,0.35) 44%, rgba(150,170,220,0.22) 52%, rgba(170,190,230,0.12) 60%, rgba(190,210,240,0.06) 72%, rgba(210,225,250,0.02) 88%, #fff 100%)' : '#fff', 
        boxShadow: scrolled ? '0 2px 8px rgba(0,0,0,.1)' : 'none',
        borderBottom: 'none',
      }}>
        {/* ── Top Bar (hidden when scrolled on desktop, always stays on mobile) ── */}
        {(!scrolled || isMobile) && (
          isMobile ? (
            /* Mobile Top Bar with Gradient Fade */
            <div style={{ background: '#fff', paddingBottom: 12 }}>
              {/* Flipkart, Travel, Grocery Row */}
              <div style={{ display: 'flex', gap: 8, alignItems: 'center', padding: '12px 12px 8px' }}>
                <Link href="/" style={{
                  background: '#ffe500', padding: '6px 12px', borderRadius: 8,
                  fontSize: 13, fontWeight: 700, textDecoration: 'none', display: 'flex', alignItems: 'center', gap: 4,
                  color: '#212121', flexShrink: 0,
                }}>
                  <span style={{ background: '#2874f0', color: '#fff', width: 18, height: 18, borderRadius: 3, display: 'inline-flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, fontWeight: 900, fontStyle: 'italic' }}>f</span>
                  <span>Flipkart</span>
                </Link>
                <button style={{ background: '#fff', border: 'none', padding: '6px 12px', borderRadius: 20, fontSize: 12, color: '#212121', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 4, fontWeight: 500 }}>
                  <span style={{ fontSize: 13 }}>✈️</span> Travel
                </button>
                <button style={{ background: '#fff', border: 'none', padding: '6px 12px', borderRadius: 20, fontSize: 12, color: '#212121', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 4, fontWeight: 500 }}>
                  <span style={{ fontSize: 13 }}>🛒</span> Grocery
                </button>
              </div>

              {/* Address Box with Coins Outside */}
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, margin: '0 12px', marginTop: 8 }}>
                <div onClick={() => setShowAddressPicker(true)} style={{ 
                  display: 'flex', alignItems: 'center', gap: 8, flex: 1,
                  padding: '8px 10px', border: '2px solid #42a5f5', borderRadius: 10,
                  background: 'rgba(255,255,255,.1)', cursor: 'pointer', fontSize: 12, color: '#212121'
                }}>
                  <svg width="16" height="16" fill="#42a5f5" viewBox="0 0 24 24"><path d="M12 2C8.13 2 5 5.13 5 9c0 5.25 7 13 7 13s7-7.75 7-13c0-3.87-3.13-7-7-7zm0 9.5c-1.38 0-2.5-1.12-2.5-2.5s1.12-2.5 2.5-2.5 2.5 1.12 2.5 2.5-1.12 2.5-2.5 2.5z"/></svg>
                  
                  <div style={{ flex: 1, minWidth: 0, lineHeight: 1.2 }}>
                    <div style={{ fontWeight: 600, fontSize: 12 }}>{userAddress?.pincode || 'Add'}</div>
                    {userAddress && <div style={{ fontSize: 11, opacity: 0.7, lineHeight: 1 }}>{userAddress.city || userAddress.locality || ''}</div>}
                  </div>
                  
                  <svg width="14" height="14" fill="#42a5f5" viewBox="0 0 24 24">
        ...
        {/* ── Search Bar Row ── */}
        <div style={{ maxWidth: 1200, margin: '0 auto', padding: scrolled ? '6px 16px' : (isMobile ? '8px 12px' : '0 16px 8px'), display: 'flex', alignItems: 'center', gap: isMobile ? 8 : 16, flexWrap: isMobile ? 'wrap' : 'nowrap', background: isMobile ? '#fff' : 'transparent' }}>
          {/* Logo in scrolled mode (desktop only) */}
          {scrolled && !isMobile && (
            <Link href="/" style={{
        ...
        {/* ── Category Icon Tabs ── */}
        <div style={{ borderTop: '1px solid #f0f0f0', overflow: 'hidden', background: isMobile ? '#fff' : 'transparent' }}>
